...
const UserLogin = () => {
  ...
  let ordersave = async OrderId => {
    await AuthGet('order/get-paydata/' + OrderId, 'customer_token').then(
      res => {
        ...
        .then(async resp => {
          if (resp.data.statusCode === 200) {
            ...
            setUserData({
              ...
              bonusPoints: resp.data.user.bonus_points
            })
            const userOrderDetails = JSON.parse(
              localStorage.getItem('userOrderDetails')
            )
            debugger
    ...
    <section className=' container-md PaddingTop'>
      ...
      {show && (
        <div>
          <div>
            <span>
              your last transaction was not completed successfully do you want
              to purchase again
            </span>
          </div>
          <button
