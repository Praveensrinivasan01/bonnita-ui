   ...
   <button onClick={()=>{ handleOkay()} } >Yes</button> <button onClick={()=>{ handleClose()}}>No</button>
      ...
      <div className="row d-flex gap-4 justify-content-center mb-5">
        ...
        <div className="col-md-5 border-1 p-4 order-md-1 order-0">
          ...
          <form className=" mt-4" onSubmit={!accountVerified ? getOtp : verifyOtp}>
            <div>
              <label className="">{!accountVerified ? "Phone Number" : "OTP"}</label>
              <input
                type="input"
                name={!accountVerified ? "mobile" : "otp"}
                placeholder={!accountVerified ? "Enter Phone Number" : "Enter OTP"}
                value={!accountVerified ? mobile : otp}
                onChange={(e) => {!accountVerified ? setMobile(e.target.value.replace(/[^0-9\\.]+/g, '')) : setOtp(e.target.value.replace(/[^0-9\\.]+/g, ''))}}
                className="w-full px-3 py-3 mt-2 border focus:border-blue-500 focus:bg-white focus:outline-none"
                autofocus
                autocomplete={false}
                required
                maxLength={!accountVerified ? 10 : 4}
              />
            </div>
            {/* <div className="mt-4">
            ...
            <div className="col-12 flex  justify-between">
                <div className="cursor-pointer" onClick={()=> setAccountVerified(!accountVerified)}>
                    Enter Number, Again?
                </div>
            ...
            <button type="submit" className="button1 color-2 mt-4 w-100">
                {!accountVerified ? "Get OTP" : "Verify OTP"}
            </button>
