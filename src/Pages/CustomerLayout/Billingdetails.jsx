...
const Billingdetails = () => {
  ...
  const handleSuccess = async (orderid, quantity) => {
      ...
      if (state2.id) {
        ...
        if (redeem) {
          response = await axios.post(
            ...
            { ...newOrder, total_amount: total + deliveryCharge - bonusPoints?.bonusPoints, quantity: totalQuantity }
          );
        } else {
          response = await axios.post(
            `${process.env.REACT_APP_API_URL}/order/create-order`,
            { ...newOrder, total_amount: total + deliveryCharge - discount, quantity: totalQuantity }
          );
        }
  ...
  const handleButtonClick = (total, totalQuantity) => {
    ...
    const requiredFields = ["room_no", "address_line1", "address_line2", "city", "country", "state", "zip_code"];
                    ...
                    <div className="sidebar sidebar-order m-0 w-100 ps-3 pe-4">
                      ...
                      <div className=" mt-3">
                        <div className="d-flex justify-content-between">
                          ...
                          <span className="price">₹{coupon === true ? total - Number(discount) + Number(deliveryCharge) : redeem === true ? total - Number(bonusPoints?.bonusPoints) + Number(deliveryCharge) : total + Number(deliveryCharge)}</span>
                        </div>
                        <div className="mt-3">
                          <h5>Payments</h5>
                          <div className="form-check mt-3">
                    ...
                    <div className="bottom-checkout d-flex justify-content-between">
                      ...
                      <div className="btn-center">
                        <button
                          ...
                          onClick={() => (handleButtonClick(total, totalQuantity), setNewOrder({
                            ...newOrder, total_amount: total, quantity: totalQuantity
                          }))}
                        >
                          NEXT STEP
                        </button>
