...
const SubCategories = () => {
  ...
  const getAllcatogories = async () => {
    const res = await axios.get(
      ...
      }/admin/get-all-subcategory/${id}?search=${searchParam}&offset=${
        15 * page
      } `,
      'admin'
    )
  ...
  const handleDelete = async e => {
    let response = await axios.delete(
      `${process.env.REACT_APP_API_URL}/product/delete-subcategory/${e}`
    )
  ...
  const handleUpload = async () => {
    ...
    if (!description) {
      toast.error('Please enter subcategory description')
      return
    }
      ...
      <div class='sm:flex sm:items-center sm:justify-between'>
        <div>
          ...
          <p class='mt-1 text-sm text-gray-500'>
            A List Of All Sub Categories Under "Category"
          </p>
        </div>
      </div>
                  ...
                  {catogories.map((catogories, index) => (
                    <tr key={index}>
                      ...
                      <td className='px-4 py-4 text-sm '>
                        <div>
                          <h4 className='text-gray-700  break-all 	w-full	'>
          ...
          <ModalBody>
            <div>
              ...
              <input
                ...
                value={name}
                // onChange={(e) => {
                //   handleInputValidation(e.target.value, setName, 0);
                // }}
