...
const Coupon = () => {
  ...
  const AssignCoupon = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/admin/add-usercoupon`,
        body
      )
      if (response?.data?.statusCode === 200) {
        setAddCouponmodal(false)
  ...
  const getAssignedUsers = async item => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/admin/assigned-coupon`,
        body
      )
      // if (response?.data?.statusCode === 200) {
      ...
      setUserCoupon(response?.data?.data.map(item => item.id) || [])
