...
const Orders = () => {
    ...
    const getAllOrders = async () => {
        const usersResponse = await axios.post(`${process.env.REACT_APP_API_URL}/admin/get-all-orders?offset=${(page) * 15}&search=${searchParam}&status=${filterParam}`, {}, 'admin')
            ...
            <div class="sm:flex sm:items-center sm:justify-between">
                <div>
                    <div class="flex items-center gap-x-3">
                        ...
                        <span class="px-3 py-1 text-xs text-red-400 bg-gray-100 rounded-full ">240</span>
                    </div>
                                    ...
                                    {data?.map((ele, idx) => {
                                        return (<tr>
                                            <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                                <div>
                                                    <h2 class="font-medium text-gray-800 ">{ele?.order_id}</h2>
                                                </div>
                                            </td>

                                            <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                                <div>
                                                    <h2 class="font-medium text-gray-800 ">{ele?.username}</h2>
                                                    <p class="text-sm font-normal text-gray-600">{ele?.email}</p>
                                                </div>
                                            </td>

                                            <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                                <div>
                                                    <h2 class="font-medium text-gray-800 ">{ele?.quantity}</h2>
                                                </div>
                                            </td>

                                            <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                                <div>
                                                    <h2 class="font-medium text-gray-800 ">₹ {ele?.total}</h2>
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-4 py-4 text-sm whitespace-nowrap flex gap-2">
                                                <Link to={`/admin/orders/${ele?.order_id}`}>
                                                    <button type="button" class="px-3 py-2 text-sm font-medium text-center rounded-none text-white bg-slate-900 flex  focus:ring-4 focus:outline-none focus:ring-slate-300 ">Details</button>
                                                </Link>
                                            </td>
                                        </tr>)
                                    })}
