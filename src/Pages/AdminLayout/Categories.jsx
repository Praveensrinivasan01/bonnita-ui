...
const Categories = () => {
  ...
  const getcatogories = async () => {
    ...
    const res = await axios.post(
      ...
      }/admin/get-all-category?search=${searchParam}&offset=${15 * page} `,
      'admin'
    )
  ...
  let editCategory = categories => {
    ...
    console.log('categories.catogoriesDescription::: ', categories)
    setFile()
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (editmode) {
        // If in edit mode, include category_id in the body
        body.category_id = editid
        ...
        // If not in edit mode, include category_image_id if file is uploaded
        if (!file) {
        ...
        if (response.data.statusCode === 200) {
          body.category_image_id = response.data.image.id
